...
const ex1 = require('./dns_profile.json');
const ex2 = require('./dns_profile2.json');

let json;

describe('DNS_Profile: ltm profile dns', () => {
    it('ex1', async () => {
        const data = await readFiles(['./test/engines/as3Converter/dns_profile/dns_profile.conf']);
    ...
    it('ex2', async () => {
        const data = await readFiles(['./test/engines/as3Converter/dns_profile/dns_profile2.conf']);
        const parsed = parse(data);
        json = as3Converter(parsed).declaration;
