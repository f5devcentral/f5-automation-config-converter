...
const ex1 = require('./nat_policy.json');
const ex2 = require('./nat_policy2.json');

let json;

describe('NAT_Policy: security nat policy', () => {
    it('ex1', async () => {
        const data = await readFiles(['./test/engines/as3Converter/nat_policy/nat_policy.conf']);
    ...
    it('ex2', async () => {
        const data = await readFiles(['./test/engines/as3Converter/nat_policy/nat_policy2.conf']);
        const parsed = parse(data);
        json = as3Converter(parsed).declaration;
