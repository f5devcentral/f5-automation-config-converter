...
const ex1 = require('./monitor_postgresql.json');
const ex2 = require('./monitor_postgresql2.json');

let json;

describe('Monitor PostgreSQL: ltm monitor postgresql', () => {
    it('ex1', async () => {
        const data = await readFiles(['./test/engines/as3Converter/monitor_postgresql/monitor_postgresql.conf']);
    ...
    it('ex2', async () => {
        const data = await readFiles(['./test/engines/as3Converter/monitor_postgresql/monitor_postgresql2.conf']);
        const parsed = parse(data);
        json = as3Converter(parsed).declaration;
