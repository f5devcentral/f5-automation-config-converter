...
const ex1 = require('./analytics_profile.json');
const ex2 = require('./analytics_profile2.json');

let json;

describe('Analytics_Profile: ltm profile analytics', () => {
    it('ex1', async () => {
        const data = await readFiles(['./test/engines/as3Converter/analytics_profile/analytics_profile.conf']);
    ...
    it('ex2', async () => {
        const data = await readFiles(['./test/engines/as3Converter/analytics_profile/analytics_profile2.conf']);
        const parsed = parse(data);
        json = as3Converter(parsed).declaration;
