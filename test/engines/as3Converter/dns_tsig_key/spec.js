...
const ex1 = require('./dns_tsig_key.json');
const ex2 = require('./dns_tsig_key2.json');

let json;

describe('DNS_TSIG_Key: ltm dns tsig-key', () => {
    it('ex1', async () => {
        const data = await readFiles(['./test/engines/as3Converter/dns_tsig_key/dns_tsig_key.conf']);
    ...
    it('ex2', async () => {
        const data = await readFiles(['./test/engines/as3Converter/dns_tsig_key/dns_tsig_key2.conf']);
        const parsed = parse(data);
        json = as3Converter(parsed).declaration;
        ...
        assert.strictEqual(originalDec.algorithm, convertedDec.algorithm);
        assert.strictEqual(originalDec.secret.protected, convertedDec.secret.protected);
    });
