...
const ex1 = require('./persist_universal.json');
const ex2 = require('./persist_universal2.json');

let json;

describe('Persist (Universal): ltm persistence universal', () => {
    it('ex1', async () => {
        const data = await readFiles(['./test/engines/as3Converter/persist_universal/persist_universal.conf']);
    ...
    it('ex2', async () => {
        const data = await readFiles(['./test/engines/as3Converter/persist_universal/persist_universal2.conf']);
        const parsed = parse(data);
        json = as3Converter(parsed).declaration;
