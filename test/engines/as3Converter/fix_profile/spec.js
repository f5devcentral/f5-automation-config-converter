...
const ex1 = require('./fix_profile.json');
const ex2 = require('./fix_profile2.json');

let json;

describe('FIX_Profile: ltm profile fix', () => {
    it('ex1', async () => {
        const data = await readFiles(['./test/engines/as3Converter/fix_profile/fix_profile.conf']);
    ...
    it('ex2', async () => {
        const data = await readFiles(['./test/engines/as3Converter/fix_profile/fix_profile2.conf']);
        const parsed = parse(data);
        json = as3Converter(parsed).declaration;
