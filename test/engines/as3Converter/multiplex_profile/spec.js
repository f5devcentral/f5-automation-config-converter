...
const ex1 = require('./multiplex_profile.json');
const ex2 = require('./multiplex_profile_full.json');

let json;

describe('Multiplex_Profile: ltm profile one-connect', () => {
    it('ex1', async () => {
        const data = await readFiles(['./test/engines/as3Converter/multiplex_profile/multiplex_profile.conf']);
    ...
    it('ex2', async () => {
        const data = await readFiles(['./test/engines/as3Converter/multiplex_profile/multiplex_profile_full.conf']);
        const parsed = parse(data);
        json = as3Converter(parsed).declaration;
