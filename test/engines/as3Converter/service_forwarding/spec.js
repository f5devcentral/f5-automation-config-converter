...
const ex1 = require('./service_forwarding.json');
const ex2 = require('./service_forwarding2.json');

let json;

describe('Service_Forward: ltm virtual', () => {
    it('ex1', async () => {
        const data = await readFiles(['./test/engines/as3Converter/service_forwarding/service_forwarding.conf']);
    ...
    it('ex2', async () => {
        const data = await readFiles(['./test/engines/as3Converter/service_forwarding/service_forwarding2.conf']);
        const parsed = parse(data);
        json = as3Converter(parsed).declaration;
