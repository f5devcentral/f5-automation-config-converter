...
const ex1 = require('./udp_profile.json');
const ex2 = require('./udp_profile_full.json');

let json;

describe('UDP_Profile: ltm profile udp', () => {
    it('ex1', async () => {
        const data = await readFiles(['./test/engines/as3Converter/udp_profile/udp_profile.conf']);
    ...
    it('ex2', async () => {
        const data = await readFiles(['./test/engines/as3Converter/udp_profile/udp_profile_full.conf']);
        const parsed = parse(data);
        json = as3Converter(parsed).declaration;
