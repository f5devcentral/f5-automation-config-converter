...
const ex1 = require('./service_udp.json');
const ex2 = require('./service_udp2.json');

let json;

describe('Service UDP: ltm virtual', () => {
    it('ex1', async () => {
        const data = await readFiles(['./test/engines/as3Converter/service_udp/service_udp.conf']);
    ...
    it('ex2', async () => {
        const data = await readFiles(['./test/engines/as3Converter/service_udp/service_udp2.conf']);
        const parsed = parse(data);
        json = as3Converter(parsed).declaration;
