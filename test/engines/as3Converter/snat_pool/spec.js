...
const ex1 = require('./snat_pool.json');
const ex2 = require('./snat_pool2.json');

let json;

describe('SNAT_Pool: ltm snatpool', () => {
    it('ex1', async () => {
        const data = await readFiles(['./test/engines/as3Converter/snat_pool/snat_pool.conf']);
    ...
    // fix bug: wrong lsn reference
    it('ex2', async () => {
        const data = await readFiles(['./test/engines/as3Converter/snat_pool/snat_pool2.conf']);
        const parsed = parse(data);
        json = as3Converter(parsed).declaration;
