...
const ex1 = require('./log_publisher.json');
const ex2 = require('./log_publisher2.json');

let json;

describe('Log_Publisher: sys log-config publisher', () => {
    it('ex1', async () => {
        const data = await readFiles(['./test/engines/as3Converter/log_publisher/log_publisher.conf']);
    ...
    it('ex2', async () => {
        const data = await readFiles(['./test/engines/as3Converter/log_publisher/log_publisher2.conf']);
        const parsed = parse(data);
        json = as3Converter(parsed).declaration;
