...
const ex1 = require('./l4_profile.json');
const ex2 = require('./l4_profile2.json');

let json;

describe('L4_Profile: ltm profile fastl4', async () => {
    it('ex1', async () => {
        const data = await readFiles(['./test/engines/as3Converter/l4_profile/l4_profile.conf']);
    ...
    it('ex2', async () => {
        const data = await readFiles(['./test/engines/as3Converter/l4_profile/l4_profile2.conf']);
        const parsed = parse(data);
        json = as3Converter(parsed).declaration;
