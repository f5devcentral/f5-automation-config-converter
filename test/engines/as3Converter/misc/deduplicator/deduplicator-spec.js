...
const ex1 = require('./duplicateNames.json');
const ex2 = require('./duplicateNames2.json');

let json;

describe('Duplicate names. Check duplicates works fine.', () => {
    it('ex1', async () => {
        const data = await readFiles(['./test/engines/as3Converter/misc/deduplicator/duplicateNames.conf']);
    ...
    it('ex2', async () => {
        const data = await readFiles(['./test/engines/as3Converter/misc/deduplicator/duplicateNames2.conf']);
        const parsed = parse(data);
        json = as3Converter(parsed).declaration;
