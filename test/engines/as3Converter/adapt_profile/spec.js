...
const ex1 = require('./adapt_profile.json');
const ex2 = require('./adapt_profile2.json');

let json;

describe('Adapt_Profile: ltm profile request-adapt', () => {
    it('ex1', async () => {
        const data = await readFiles(['./test/engines/as3Converter/adapt_profile/adapt_profile.conf']);
...
describe('Adapt_Profile: ltm profile response-adapt', () => {
    it('ex2', async () => {
        const data = await readFiles(['./test/engines/as3Converter/adapt_profile/adapt_profile2.conf']);
        const parsed = parse(data);
        json = as3Converter(parsed).declaration;
