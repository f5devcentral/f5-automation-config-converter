...
const ex1 = require('./nat_source_translation.json');
const ex2 = require('./nat_source_translation_2.json');

let json;

describe('NAT_Source_Translation: security nat source-translation', () => {
    it('ex1', async () => {
        const data = await readFiles(['./test/engines/as3Converter/nat_source_translation/nat_source_translation.conf']);
    ...
    // check support exclusion addresses
    it('ex2', async () => {
        const data = await readFiles(['./test/engines/as3Converter/nat_source_translation/nat_source_translation_2.conf']);
        const parsed = parse(data);
        json = as3Converter(parsed).declaration;
