...
const ex1 = require('./log_destination.json');
const ex2 = require('./log_destination2.json');

let json;

describe('Log_Destination: sys log-config destination', () => {
    it('ex1: (remote-high-speed-log)', async () => {
        const data = await readFiles(['./test/engines/as3Converter/log_destination/log_destination.conf']);
    ...
    it('ex2: (remote-syslog)', async () => {
        const data = await readFiles(['./test/engines/as3Converter/log_destination/log_destination2.conf']);
        const parsed = parse(data);
        json = as3Converter(parsed).declaration;
