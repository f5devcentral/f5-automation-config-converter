 ...
 * @param {set} set to hold schema keys
 * @returns array created
 */
const setAllProps = (props, set) => {
    if (typeof props !== 'object') return set;
...
const isValid = (decl) => {
    ...
    const allKeys = Array.from(setAllProps(adcSchema, new Set()));

    for (let i = 0; i < declKeys.length; i += 1) {
        if (allKeys.indexOf(declKeys[i]) === -1) {
            return {
                isValid: false,
                data: declKeys[i]
            };
